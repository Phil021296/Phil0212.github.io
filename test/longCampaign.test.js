...
const go=(c,id)=>{if(c.state.location!==id)resolveChoice(c,{travelId:id,input:id});};
...
test('Beide Erweiterungskapitel einschließlich aller Nebenfälle spielbar und speicherbar',()=>{
 ...
 go(c,chapter.hub);choose(c,'long_case_'+m.id);
 ...
 go(c,chapter.hub);choose(c,'long_answer_'+((m.correct+1)%3));assert.equal(c.state.story.expansion.cases[m.id].solved,false);
 choose(c,'long_answer_'+m.correct);choose(c,m.optional?'long_quiet':'long_public');
 assert.throws(()=>choose(c,'long_public'));
 }
 choose(c,'long_finish');
 ...
 assert.ok(c.state.story.journal.length>=72);
});
