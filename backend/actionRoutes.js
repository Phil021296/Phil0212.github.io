...
export function mountActions(app,pool,authenticate,{provider=ai,gameData=data}={}){
 app.get('/api/gm',(_req,res)=>res.json({enabled:provider.configured,version:'1.1.0'}));
 app.post('/api/action',authenticate,async(req,res)=>{
  const {input,requestId,revision}=req.body||{};
  ...
  if(!provider.configured)return res.status(503).json({error:'ai_not_configured'});
  let client;
  try{
   ...
   if(receipt.rows.length){await client.query('COMMIT');if(receipt.rows[0].input!==input)return res.status(409).json({error:'request_id_reused'});return res.json({state:current.state,revision:Number(current.revision),savedAt:current.updated_at,replayed:true});}
   if(Number(current.revision)!==revision){await client.query('ROLLBACK');return res.status(409).json({error:'revision_conflict'});}
   const state=await runTurn({state:current.state,input,data:gameData,ai:provider});
   const saved=await client.query('UPDATE savegames SET state=$1::jsonb,game_version=$2,revision=revision+1,updated_at=NOW() WHERE player_id=$3 RETURNING revision,updated_at',[JSON.stringify(state),'1.1.0',req.playerId]);
   await client.query('INSERT INTO gm_requests(player_id,request_id,input) VALUES($1,$2,$3)',[req.playerId,requestId,input]);
